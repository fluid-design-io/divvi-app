...
type ListDataItem = string | { title: string; subTitle?: string; tertiaryText?: string };
...
type ListItemProps<T extends ListDataItem> = PressableProps &
  ListRenderItemProps<T> & {
    ...
    tertiaryTextStyle?: StyleProp<TextStyle>;
    tertiaryTextNumberOfLines?: number;
...
const ESTIMATED_ITEM_HEIGHT = {
  titleOnly: Platform.select({ ios: 45, default: 57, web: 57 }),
  withSubTitle: Platform.select({ ios: 56, default: 57, web: 57 }),
  withTertiaryText: 64,
  withValue: 72,
};
...
/**
 * An extended list data item that allows for more flexibility in the list items.
 */
type ExtendedListDataItem =
  | string
  | {
      id: string;
      title: string;
...
export type { ExtendedListDataItem };

...
export const GroupMembers = ({ members }: { members?: GroupMember[] }) => {
  const openSheet = () => {};
  return (
    <View className="mt-4 w-full rounded-lg bg-card">
      <ScrollView
        horizontal
        contentContainerClassName="gap-4 p-4"
        showsHorizontalScrollIndicator={false}>
        {members?.map((member) => (
          <GroupMemberAvatar key={`member-${member.id}`} member={member} />
        ))}
...
const GroupMemberAvatar = ({ member }: { member: GroupMember }) => {
  return (
